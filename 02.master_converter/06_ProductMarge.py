...
df['Production LT_s'] = df['Production LT_s'].astype(int)
df['DaysTS'] = df['DaysTS'].astype(int)

dfs = pd.DataFrame()
dfs['Process Mode'] = '2'  # 処理区分
dfs['Master ID'] = '01'  # 登録区分
...
df['Cutoff Time for 1day MTO'] = df['GTI_Order_Close']  # 当日受注締時刻
...
dfs['Days to Ship on Catalog'] = df['Production LT_s'] + df['DaysTS']  # カタログ納期
...
for i in range(0, n):
    df = dfs.loc[[i]]
    a = np.array(df['Stock / MTO'])
    b = np.array(df['位置'])
    c = np.array(df['Weight2'])
    d = np.array(df['Weight_y'])
    e = np.array(dfs['Weight Calc_y'])

    if a == '0' and b == 'x':
        df['Weight'] = c
    elif a == '0' and d > '0' or a == '1':
        df['Weight'] = d
    else:
        df['Weight'] = e
...
dfs.to_csv(path + 'dfs_20190819_Product.txt', sep='\t', encoding='utf_16', index=False)  # Zetta_Product.txt　ALL出力
df.to_csv(path + 'df_20190819_Product.txt', sep='\t', encoding='utf_16', index=False)  # Zetta_Product.txt　ALL出力
print('fin')