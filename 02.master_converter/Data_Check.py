...
ac = pd.DataFrame(pd.read_csv(path + '03722108_USA_UnitPrice_filled.txt',sep='\t', encoding='utf_16', dtype=str, engine='python', error_bad_lines=False))   # 文字列で読み込み
...
py = pd.DataFrame(pd.read_excel(path + 'USA_U_Product.xlsx', sheet_name='Sheet1', dtype=str))    # 文字列で読み込み
...
for i in range(1, l):
    ...
    print(i)
df = df.loc[:, h]
df.to_excel(path + 'spc_usa_check.xlsx', index=False)  # 検証用
print(datetime.datetime.now())
print('fin')