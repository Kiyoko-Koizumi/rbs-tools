...
u_check = u_check[['Subsidiary Code', 'Product Code']]
...
for l in range(0, n):
    ...
    spc_data = pd.merge(spc, u_check, on=['Product Code'])  # SZ_Slideデータの型式が一致する現法データ
...
for l in range(0, n):
    ...
    z_data = pd.merge(z, u_check, on=['Subsidiary Code', 'Product Code'])  # SZ_Slideデータの現法コードと型式が一致する現法データ
