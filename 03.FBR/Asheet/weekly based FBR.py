...
fixed_calendar = fixed_calendar.loc[:, ['稼働日','日付_y', 'Supplier' ]]
...
calendar = calendar.loc[:, ['日付_y', '稼働日', '管理Gr']]
calendar = calendar.rename(columns={'稼働日': '稼動'})
...
# ファイルアウトプット
f_name = '③FBR需給一覧(SD-33371)_モニター用_日次.tsv'
export_excel = FBR_SUM.to_excel(f_name, index = None, header=True)
