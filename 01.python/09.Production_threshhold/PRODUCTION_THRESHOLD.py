...
if __name__ == '__main__':
    ...
    # ファイル名を取得
    file = glob.glob('PRODUCTION_THRESHOLD_*.csv')
    file_name = os.path.basename(file[0])
    file_cd = file_name.replace('PRODUCTION_THRESHOLD_', '')
    file_cd = file_cd.replace('.csv', '')

    # PRODUCTION_THRESHOLD.csvデータ取り込み
    THRESHOLD = pd.read_csv(file_name, encoding=font, dtype='object', index_col=None)
    # 開始日と終了日を取得
    ...
    THRESHOLD_SUM.to_csv(f_name, sep='\t', encoding=font, quotechar='"', line_terminator='\n', index=False)

    # ファイルアウトプット
    f_name = 'pcd_' + file_cd + '.tsv'
    PROCESSED_DATA.to_csv(f_name, sep='\t', encoding=font, quotechar='"', line_terminator='\n', index=False)
