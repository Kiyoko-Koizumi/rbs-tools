...
data = pd.merge(data, jri_cost, on='番号', how='left')
# 従来が3764,0FCNでないR.B.S候補は削除 nullの場合は見積りデータなので残す
data = data[(((data['従来拠点'] == '7017') & (data['RBS_受注現法仕入先コード'] == '7017')) |
             ((data['従来拠点'] == 'SPCM') & (data['RBS_受注現法仕入先コード'] == 'SPCM')) |
             ((data['従来拠点'] == '3764') & ((data['RBS_受注現法仕入先コード'] == '3764') | (data['RBS_受注現法仕入先コード'] == '0FCN'))) |
             ((data['従来拠点'] == '0FCN') & ((data['RBS_受注現法仕入先コード'] == '3764') | (data['RBS_受注現法仕入先コード'] == '0FCN'))) |
               data['従来拠点'].isnull())]
