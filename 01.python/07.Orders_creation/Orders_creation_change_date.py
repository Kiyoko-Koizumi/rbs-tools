...
def read_so():
    ...
    for r in range(1, len(list_f)):
        ...
        print(f_name)
        order_add = pd.read_csv(f_name, sep='\t', encoding=font, dtype=object, engine='python', error_bad_lines=False)
        # ファイルを追加する
    ...
    order.loc[:, '顧客希望納期'] = order['顧客希望納期'].str[0:4] + '-' + order['顧客希望納期'].str[4:6] + '-' + order['顧客希望納期'].str[6:8]
...
def noki_zokusei(i, order, sub_name, calendar_dict):
    ...
    # 現法毎に納期属性をつける
    df.loc[:, '納期属性'] = [(z - y).days - len(list(filter(lambda x: y <= x <= z, calendar_dict[sub_name[i]]))) for y, z in zip(df['受注日'], df['顧客希望納期'])]
    return df
...
if __name__ == '__main__':
    ...
    # 各現法、拠点の非稼働日をリスト化
    calendar_dict = holidays(sub_name)
