...
def getFACI_CD():
    ...
    button = tk.Button(root, text="select",command=lambda: getitemcode(combo_l[0].get(), combo_l[1].get(), combo_l[2].get(), combo_l[3].get(), combo_l[4].get(), combo_l[5].get(), combo_l[6].get(),
                                                                       combo_l[7].get(), combo_l[8].get(), combo_l[9].get(), combo_l[10].get(), combo_l[11].get(), combo_l[12].get()))
    # ボタンの配置
...
def Sub_func(s, Pre_S, Pre_E, calendar_dict, sub_name, pg_name):
    prediction = pd.DataFrame({'現法コード': [], '受注日': [], '受注日稼働flg': [], '出荷日': [], '出荷日稼働flg': [], '納期属性': []})
    count = 0
    for d in range(0, (Pre_E - Pre_S).days + 1):
        SO_day = Pre_S + dt.timedelta(days=d)
        print(sub_name[s] + '/' + SO_day.strftime('%Y%m%d'))
        if SO_day.strftime('%Y-%m-%d') in calendar_dict[sub_name[s]]:
            SO_workday = 0
        else:
            SO_workday = 1
        nouki_count = 0
        for k in range(0, 73):
            ...
            if SD_day.strftime('%Y-%m-%d') in calendar_dict[pg_name]:
                SD_workday = 0
            else:
                SD_workday = 1
            prediction.loc[count] = [sub_name[s], SO_day, SO_workday, SD_day, SD_workday, nouki_count]
            count += 1
            nouki_count = nouki_count + SD_workday
    #global pre_dict
    #pre_dict[sub_name[s]] = prediction
    return prediction

def pre_sum1(d, Pre_S, pre_b, count0, prediction):
    print(d)
    SO_day = Pre_S + dt.timedelta(days=d)
    for k in range(0, 73):
        SD_day = SO_day + dt.timedelta(days=k)
        # 当日分除く場合は下記prediction['受注日'] >= SO_day　→　prediction['受注日'] > SO_dayへ
        pre_a = prediction[prediction['受注日'] > SO_day]
        pre_a = pre_a[pre_a['出荷日'] == SD_day.strftime('%Y-%m-%d')]
        pre_qty = pre_a.groupby(['出荷日'], as_index=True)['数量'].sum()
        if pre_qty.empty:
            Qty = 0
        else:
            Qty = pre_qty[0]
        pre_b.loc[count0] = [SO_day.strftime('%Y-%m-%d'), SD_day.strftime('%Y-%m-%d'), Qty]
        count0 += 1
    return pre_b

def pre_sum2(d, Pre_S, FACILITY_L, prediction):
    print(d)
    pre_c = pd.DataFrame({'SUBSIDIARY_CD': [], 'SUPPLIER_CD': [], 'FACILITY_CD_AL': [], 'BASE_DATE': [], 'BASE_DATE_ADD_DAYS': [], 'PREDICTION_QUANTITY': [], 'UPD_COUNT': [], 'DEL_FLG': [], 'REG_USR': [], 'REG_TIME': [], 'UPD_USR': [], 'UPD_TIME': []})
    ...
    for k in range(0, 73):
        ...
        pre_a = pre_a[pre_a['出荷日'] <= SD_day.strftime('%Y-%m-%d')]
        pre_qty = pre_a.groupby(['受注日'], as_index=True)['数量'].sum()
        if pre_qty.empty:
            Qty = 0
        else:
            Qty = pre_qty[0]
        pre_c.loc[count1] = [FACILITY_L[0], FACILITY_L[1], FACILITY_L[2], SO_day.strftime('%Y%m%d'), k, Qty, '0', '0', None, Today, None, Today]
        count1 += 1
    return pre_c
...
if __name__ == '__main__':
    ...
    # 必要な列のみ読み込む
    print(f_name)
    order = pd.read_csv(f_name, sep='\t', encoding=font, dtype=object, engine='python', error_bad_lines=False, usecols=['番号', '現法コード', 'グローバル番号', '受注日・見積回答日', '受注時間・見積回答時間', 'JST変換受注日・JST変換見積回答日', 'JST変換受注時間・JST変換見積回答時間', '見積有効日', '見積有効時間', 'JST変換見積有効日', 'JST変換見積有効時間', 'アンフィット種別', '得意先コード', '直送先コード', 'ＭＣコード', 'インナーコード', '商品コード', '実績現法コード', '実績仕入先コード', '実績管理単位コード', 'ACE仕入先コード', 'ACE仕入先カテゴリコード', '受注実績SSD', '見積回答SSD', '数量', '納入区分', '顧客希望納期'])

    # ファイルを繰り返し開き結合する
    for r in range(1,len(list_f)):
        f_name=os.path.basename(list_f[r])
        print(f_name)
        order_add = pd.read_csv(f_name, sep='\t', encoding=font, dtype=object, engine='python', error_bad_lines=False, usecols=['番号', '現法コード', 'グローバル番号', '受注日・見積回答日', '受注時間・見積回答時間', 'JST変換受注日・JST変換見積回答日', 'JST変換受注時間・JST変換見積回答時間', '見積有効日', '見積有効時間', 'JST変換見積有効日', 'JST変換見積有効時間', 'アンフィット種別', '得意先コード', '直送先コード', 'ＭＣコード', 'インナーコード', '商品コード', '実績現法コード', '実績仕入先コード', '実績管理単位コード', 'ACE仕入先コード', 'ACE仕入先カテゴリコード', '受注実績SSD', '見積回答SSD', '数量', '納入区分', '顧客希望納期'])
        #ファイルを追加する
    ...
    # 納期属性を計算
    order['納期属性'] = [(z - y).days - len(list(filter(lambda x: y <= x <= z, nowork_day_w))) for y,z in zip(order['受注日'], order['受注実績SSD'])]
    ...
    order_subtotal['開始日'] = pd.to_datetime((order_subtotal['year'] + '/' + order_subtotal['month'] + '/01'), format='%Y/%m/%d')
    order_subtotal['終了日'] = order_subtotal['開始日'] + offsets.MonthBegin(1)
    order_subtotal['月稼働日'] = [(z - y).days - len(list(filter(lambda x: y <= x < z, nowork_day_w))) for y,z in zip(order_subtotal['開始日'], order_subtotal['終了日'])]
    ...
    FC['月稼働日'] = [(z - y).days - len(list(filter(lambda x: y <= x < z, nowork_day_w))) for y, z in zip(FC['開始日'], FC['終了日'])]
    FC['月平均本数'] = (FC['FC']/FC['月稼働日']).round(3)
    FC = FC.astype({'year': int, 'month': int})

    f_name = pg_name + '_FC.tsv'
    FC.to_csv(f_name, sep='\t', encoding=font, quotechar='"', line_terminator='\n', index=False)
    ...
    f_name = pg_name + '_week.tsv'
    order_week1.to_csv(f_name, sep='\t', encoding=font, quotechar='"', line_terminator='\n', index=False)
    ...
    f_name = pg_name + '_base_sh.tsv'
    base_sh.to_csv(f_name, sep='\t', encoding=font, quotechar='"', line_terminator='\n', index=False)

    f_name = pg_name + '_order.tsv'
    order.to_csv(f_name, sep='\t', encoding=font, quotechar='"', line_terminator='\n', index=False)
    ...
    list = [(x, Pre_S, Pre_E, calendar_dict, sub_name, pg_name) for x in range(14)]
    pre_list = pool.map(wrapper, list)
    pool.close()
    ...
    prediction.index.names = ['year', 'month', '受注日']
    ...
    # 現法毎日当たり数量を算出
    prediction['数量'] = prediction['受注日稼働flg'] * prediction['出荷日稼働flg'] * prediction['月平均本数'] * prediction['week_ratio'] * prediction['n_ratio'] * prediction['small_ratio']
    ...
    pre_1 = pd.DataFrame({'受注日': [], '出荷日': [], '数量': []})
    # q*20パート
    for s in range(0, q):
        pre_b = pd.DataFrame({'受注日':[], '出荷日':[], '数量':[]})
        count0 = 0
        pool = Pool(multi.cpu_count()-2)
        list2 = [(d, Pre_S, pre_b, count0, prediction) for d in range((20*s), (20*s+20))]
        prediction_sum1 = pool.map(wrapper2, list2)
        pool.close()
        for d in range(0, 20):
            pre_1 = pre_1.append(prediction_sum1[d], sort=False)
    # modパート
    pre_b = pd.DataFrame({'受注日': [], '出荷日': [], '数量': []})
    count0 = 0
    pool = Pool(multi.cpu_count() - 2)
    list2 = [(d, Pre_S, pre_b, count0, prediction) for d in range((20 * q), (20 * q + mod))]
    prediction_sum2 = pool.map(wrapper2, list2)
    pool.close()
    for d in range(0, mod):
        pre_1 = pre_1.append(prediction_sum2[d], sort=False)

    pre_1.reset_index(drop=True, inplace=True)
    prediction = pre_1
    FACILITY_DICT = {}
    ...
    for s in range(0, q):
        pool = Pool(multi.cpu_count() - 2)
        list3 = [(d, Pre_S, FACILITY_L, prediction) for d in range((20*s), (20*s+20))]
    ...
    # modパート
    pool = Pool(multi.cpu_count() - 2)
    list3 = [(d, Pre_S, FACILITY_L, prediction) for d in range((20 * q), (20 * q + mod))]
