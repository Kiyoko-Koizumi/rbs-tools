...
def supp_couont(d, sub_name, calendar_dict, threshold):
    ...
    if len(threshold):
        ...
        threshold = threshold.astype(
            {'PRODUCTION_ABILITY_MAX': int, 'PRODUCTION_ABILITY_MIN': int, 'DUE_DATE_DELAYED_LINE': int, 'others': int})
    ...
    threshold = threshold.loc[:, ['SUBSIDIARY_CD', 'SUPPLIER_CD', 'FACILITY_CD', 'BASE_DATE', 'PRODUCTION_ABILITY_MAX',
                                  'PRODUCTION_ABILITY_MIN', 'DUE_DATE_DELAYED_LINE']]
    return threshold
...
def PRODUCTION_THRESHOLD():
    ...
    if len(file) > 0:
        for file_count in file:
            file_name = os.path.basename(file_count)
            file_cd = file_name.replace('PRODUCTION_THRESHOLD_', '')
            file_cd = file_cd.replace('.csv', '')

            # PRODUCTION_THRESHOLD.csvデータ取り込み
            THRESHOLD = pd.read_csv(script_pass + '/Production_threshold/' + file_name, encoding=font, dtype='object',
                                    index_col=None)
            # 開始日と終了日を取得
            THRESHOLD.loc[:, '日付'] = pd.to_datetime((THRESHOLD['year'] + '/' + THRESHOLD['month'] + '/01'),
                                                    format='%Y/%m/%d')
            ...
            # 非稼働日データを読み込む
            nowork_day = pd.read_csv(script_pass + '/Production_threshold/nowork_day.csv', encoding=font,
                                     dtype='object', index_col=None)

            # 各現法、拠点の非稼働日をリスト化
            sub_name = ['CHN', 'GRM', 'HKG', 'IND', 'JKT', 'KOR', 'MEX', 'MJP', 'MYS', 'SGP', 'THA', 'TIW', 'USA',
                        'VNM', '7017', '3764', '0FCN', '0AIO', 'SPCM', '0143']
            calendar_name = ['CAAAA', 'GAAAA', 'NAAAA', 'DAAAA', 'JAAAA', 'KAAAA', 'QAAAA', '5AAAA', 'MAAAA', 'SAAAA',
                             'HAAAA', 'TAAAA', 'UAAAA', 'VAAAA', '5AAAA', '5AAAA', 'C8677', 'C8677', '50SPC', '5AAAA']
            calendar_dict = {}
            ...
            pool.close()
            THRESHOLD_SUM = pd.DataFrame({'SUBSIDIARY_CD': [], 'SUPPLIER_CD': [], 'FACILITY_CD': [], 'BASE_DATE': [],
                                          'PRODUCTION_ABILITY_MAX': [], 'PRODUCTION_ABILITY_MIN': [],
                                          'DUE_DATE_DELAYED_LINE': []})
            for d in range(6):
            ...
            PROCESSED_DATA = PROCESSED_DATA.loc[:,
                             ['SUBSIDIARY_CD', 'SUPPLIER_CD', 'FACILITY_CD', 'PROCESSED_DATE', 'PROCESSED_QUANTITY',
                              'UPD_COUNT', 'DEL_FLG', 'REG_USR', 'REG_TIME', 'UPD_USR', 'UPD_TIME']]

            # ファイルアウトプット
            f_name1 = 'cap_' + file_cd + '.tsv'
            THRESHOLD_SUM.to_csv(script_pass + '/Production_threshold/' + f_name1, sep='\t', encoding=font,
                                 quotechar='"', line_terminator='\n', index=False)

            # ファイルアウトプット
            f_name2 = 'pcd_' + file_cd + '.tsv'
            PROCESSED_DATA.to_csv(script_pass + '/Production_threshold/' + f_name2, sep='\t', encoding=font,
                                  quotechar='"', line_terminator='\n', index=False)
            print('PRODUCTION_THRESHOLD' + file_cd + 'を元に' + f_name1 + 'と' + f_name2 + 'を作成しました')
        print('PRODUCTION_THRESHOLD処理終わりました!')
    else:
        print('該当するPRODUCTION_THRESHOLDファイルがありません!')
