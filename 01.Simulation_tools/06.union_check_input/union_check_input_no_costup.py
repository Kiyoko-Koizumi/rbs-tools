...
# M単毀損なし
# 発注現法仕入値をfloatへ
data=data.astype({'発注現法仕入値':float, 'ランデットコスト':float})
jri_cost = data[data['従来生産拠点フラグ'] == '1']
jri_cost = jri_cost.rename(columns={'発注現法仕入値': '従来拠点着荷コスト', 'ランデットコスト': '従来拠点ランデットコスト'})
jri_cost = jri_cost.loc[::, ['番号', '従来拠点着荷コスト', '従来拠点ランデットコスト']]
data = pd.merge(data, jri_cost, on='番号', how='left')
# M単毀損行を削除 nullの場合は見積りデータなので残す
data = data[(((data['発注現法仕入値'] <= data['従来拠点着荷コスト']) & (data['ランデットコスト'] <= data['従来拠点ランデットコスト'])) | data['従来拠点着荷コスト'].isnull())]
data.drop(['従来拠点着荷コスト', '従来拠点ランデットコスト'], axis=1, inplace=True)
