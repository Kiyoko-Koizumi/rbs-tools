...
def Sub_func(s, so_day_list, sd_day_list, sosup_day_list, noukizokusei_list, calendar_dict, sub_name):
    # 受注日稼働flgを作成
    so_day_workflg = []
    ...
    prediction = pd.DataFrame(
        data={
            ...
            '工場受注日': sosup_day_list,
            '出荷日': sd_day_list,
            '納期属性': noukizokusei_list
        },
        columns=['受注日', '受注日稼働flg', '工場受注日', '出荷日', '納期属性']
    )
    # 現法コードを作成
...
if __name__ == '__main__':
    Orders_prediction_for_Supplier()
